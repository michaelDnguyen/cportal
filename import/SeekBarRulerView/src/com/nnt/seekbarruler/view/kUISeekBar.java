...
//import com.example.khavn.myapplication.R;

/**
 * Created by NNTai on 6/5/15.
 */

public class kUISeekBar extends View {

	public interface kUISeekBarDelegate {
		void kUISeekBarOnChangeValue(kUISeekBar view, int value);
	}
	...
	private Runnable _updateTimerThread = new Runnable() {
		...
		public void run() {
			...
			// Log("deltaStep = " + _deltaStep);
			_minDisplay += _deltaStep;
			_maxDisplay += _deltaStep;
			_currentValue += _deltaStep;

			if (_delegate != null)
				_delegate.kUISeekBarOnChangeValue(kUISeekBar.this, (int) _currentValue);
	...
	public void Log(String mess) {
		if (isDebug())
			Log.d("UISeekBar - " + debugName, mess);
	}
	...
	public void setMinValue(int value) {
		if (_minValue != value) {
			...
			if (_minDisplay < value) {
				float temp = _maxDisplay - _minDisplay;

				_minDisplay = value;
				_maxDisplay = value + temp;

				if (_maxDisplay > _maxDisplay)
					_maxDisplay = _maxValue;
			}
	...
	public void setMaxValue(int value) {
		if (_maxValue != value) {
			...
			if (_maxDisplay > value) {
				...
				_maxDisplay = value;
				_minDisplay = value - temp;

				if (_minDisplay < _minValue)
					_minDisplay = _minValue;
			}
	...
	public void setCurrentValue(float value, boolean flag) {
		isMove = flag;
		if (value < _minValue)
			value = _minValue;
	...
	public void setCurrentValue(float value) {
		this.setCurrentValue((float) value, true);
	}
	...
	// CONTRUCTORs
	public kUISeekBar(Context context, AttributeSet attrs) {
		super(context, attrs);
		...
		try {
			...
			_sliderHeight = (int) (a.getInt(R.styleable.kUISeekBar_sliderHeight, 10) * density); // default
																									// 10
																									// px
	...
	protected void onDraw(Canvas canvas) {
		...
		Display display = (Display) ((WindowManager) this.getContext().getSystemService(Context.WINDOW_SERVICE))
				.getDefaultDisplay();
		...
		_unitWidth = ((width - 2 * (_horizonPadding + _cursorRadius)) / (float) (_maxDisplay - _minDisplay)); // 1
																												// unit
																												// to
																												// pxel
																												// Log("UNIT WIDTH = "
																												...
																												// +
																												// _minDisplay);
		// Log("currentValue = " + _currentValue + " maxValue=" + _maxValue +
		// " minValue=" + _minValue);
		...
		if (_cursorImage == null) {
			a = (int) (_horizonPadding + _cursorRadius + (_currentValue - _minDisplay) * _unitWidth); // center
																										// x
		} else {
			a = (int) (_horizonPadding + (_currentValue - _minDisplay) * _unitWidth) + _cursorRadius; // center
																										// x
		}
		...
		// draw stripes
		// canvas.save();
		// canvas.clipRect(left, top, a, b);
		//
		// int offset = (int)(_textSize*2) + 10;
		// _paint.setStrokeWidth((int)(_textSize * 0.35));
		// _paint.setColor(Color.parseColor("#B22222"));
		// top -= 5;
		// for(int x = _horizonPadding - offset; x < a; x += _textSize) {
		// canvas.drawLine(x, b, x + offset, top, _paint);
		// }
		// canvas.restore();
		// end draw stripes
		...
		for (int i = mind; i <= maxd; i += _unitStep, left += unitWidth) {
			...
			float dt = i;
			String text;
			if (!_rulerUnit.equals("%")) {
				if (i >= 1000) {
					dt = dt / 1000;
					_rulerUnit = "tỷ";
					text = Float.toString(dt);
				} else {
			...
			canvas.drawText(text + (_rulerUnit != null ? _rulerUnit : ""), left - rect.width() / 2,
					top + rect.height(), _paint);
		}
		unitWidth /= _unitStep;
		canvas.restore();
		// end clip
		...
		// draw cursor seekbar
		if (_cursorImage == null) {
			left = (int) (_horizonPadding + _cursorRadius + (_currentValue - _minDisplay) * unitWidth); // center
																										// x
			...
			left = (int) (_horizonPadding + (_currentValue - _minDisplay) * unitWidth); // center
			top = _verticalPadding + _sliderHeight / 2 - _cursorRadius; // center

			temp = 2 * _cursorRadius;
			Rect sr = new Rect(0, 0, _cursorImage.getWidth(), _cursorImage.getHeight());
			Rect dr = new Rect(left, top, left + temp + temp / 2, top + temp * 2);
